...
export default function EChitPage() {
  ...
  // Added `user` object to reliably get the delegate's country
  const { user, userName, isAdmin } = useAuth(); 
  
  // Bulletproof fallback: use user.country if available, otherwise userName
  const currentCountry = isAdmin ? 'Chair' : (user?.country || userName);
  ...
  const recipients = isAdmin
    ...
    : ['Chair', ...delegates.map(d => d.country).filter(c => c !== currentCountry)];
  ...
  const filteredInbox = chits.filter(c => {
    // 1. Executive Board sees everything that passes the dropdown filter
    if (isAdmin) {
      return filterType === 'all' || c.type === filterType;
    }

    // 2. Determine if the current user is involved in this chit
    const isSender = c.from_delegate === currentCountry;
    const isReceiver = c.to_delegate === currentCountry;

    // 3. If they are neither the sender nor the receiver, hide it immediately
    if (!isSender && !isReceiver) {
      return false;
    }

    // 4. If they are the receiver, but the chit is still pending EB approval, hide it
    if (isReceiver && !isSender && c.via_eb && c.eb_status === 'pending') {
      return false;
    }

    // 5. Apply the dropdown filter (General, Question, etc.)
              ...
              <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                {sortedFilteredInbox.length === 0 ? (
                  ...
                  const canReply = replyingToId !== c.id && c.from_delegate !== 'Chair' && (
                    isAdmin || c.to_delegate === currentCountry
                  );
